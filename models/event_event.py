...
class EventEvent(models.Model):
    ...
    def button_export_all_registrations_to_csv(self):
        return {
            'type': 'ir.actions.act_url',
            'url': '/event/%s/export' % self.id,
            'target': 'self',
        }
    def export_all_registrations_to_csv(self):
        _logger.info('export_all_registrations_to_csv method called')
        ...
        for event in self:
            ...
            # Write header
            header = ['Registration ID', 'Name', 'Registration Date', 'Email', 'Phone'] + questions.mapped('title')
            writer.writerow(header)

            # Get all registrations for this event
            registrations = self.env['event.registration'].search([('event_id', '=', event.id)])

            for record in registrations:
                # Get answers for this registration
                answers = self.env['event.registration.answer'].search([('registration_id', '=', record.id)])

                # Prepare row
                row = [record.id, record.name, record.create_date, record.email, record.phone]
                for question in questions:
                    # Find the answer to this question
                    answer = answers.filtered(lambda a: a.question_id == question)

                    # Add answer to row
                    if answer:
                        row.append(
                            answer.value_text_box if answer.question_type == 'text_box' else answer.value_answer_id.name)
                    else:
                        row.append('')
        ...
        # Get string data from StringIO object
        csv_data = output.getvalue()
        output.close()

        _logger.info('export_all_registrations_to_csv method finished')
